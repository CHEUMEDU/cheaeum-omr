...
// 버전 이력
// ─────────────────────────────────────────
// v23.22 (2026-06-04) — 시험 선택 안전장치
//   ★ 학생앱 자체 중복 제거 키에 선생님·시험시간을 포함해 다른 시험 카드 병합 방지
//   ★ 빈 정답/검수대기 행이 넘어와도 시험 시작 전에 차단
...
// v23.22 (2026-06-04) — 시험 선택 안전장치
//   ★ 학생앱 자체 중복 제거 키에 선생님·시험시간을 포함
//   ★ 빈 정답/검수대기 행이 넘어와도 시험 시작 전에 차단
// v23.21 (2026-06-03) — OMR 정답조회 안전장치 + 객관식 라벨 보강
//   ★ fallback 정답조회 다중 후보는 GAS에서 ambiguous 처리해 오답 노출 방지
//   ★ "정답은 B입니다" / "Answer is B" / "Option B" 같은 명시적 보기 라벨 추가 인식
// v23.20 (2026-06-03) — OMR 객관식 정답 매칭 회귀 보강 (코드리뷰 반영)
//   ★ 끝괄호 차수 추출을 인식 태그(1차/2차/이론편/실전편/혼합/세트A~E/A~E)로 제한
//   ★ A~E→1~5 변환을 보기 라벨이 확실할 때만 (주관식 "a dog" "do" 오변환 방지)
// v23.19 (2026-06-03) — OMR 객관식 정답 매칭 보강
//   ★ ①②③④⑤ / "2번" / "정답: ②" 정답키도 OMR 숫자 답안과 일치 처리
//   ★ view_answer_key fallback 호출에 차수(setType)와 반(className) 전달
...
function normChoice(v){
  if(v===null||v===undefined||v==="")return"";
  const circleMap={"①":"1","②":"2","③":"3","④":"4","⑤":"5","➀":"1","➁":"2","➂":"3","➃":"4","➄":"5","❶":"1","❷":"2","❸":"3","❹":"4","❺":"5"};
  const uniqueSorted=tokens=>[...new Set(tokens.map(x=>String(x||"").trim()).filter(x=>/^[1-5]$/.test(x)))].sort().join(",");
  if(Array.isArray(v))return uniqueSorted(v.flatMap(x=>String(normChoice(x)).split(",")));
  const original=String(v).trim();
  if(!original)return"";
  const circled=original.match(/[①②③④⑤➀➁➂➃➄❶❷❸❹❺]/g);
  if(circled&&circled.length>0)return uniqueSorted(circled.map(c=>circleMap[c]));
  const alphaMap={a:"1",b:"2",c:"3",d:"4",e:"5"};
  // ★ v23.20: 보기 라벨이 확실할 때만 A~E → 1~5 (주관식 "do" "a dog" 오변환 방지)
  const alphaExplicit=
    original.match(/^\s*(?:정답|답)\s*(?:은|는)?\s*[:：.]?\s*([A-Ea-e])\s*(?:입니다|임|이다)?\s*$/)||
    original.match(/^\s*(?:answer|choice|option)\s*(?:is)?\s*[:：.]?\s*([A-Ea-e])\s*$/i);
  if(alphaExplicit&&alphaMap[String(alphaExplicit[1]).toLowerCase()])return alphaMap[String(alphaExplicit[1]).toLowerCase()];
  const alphaLabel=original
  ...
  const tokens=[];
  const re=/(^|[^0-9])([1-5])(?=$|[^0-9])/g;
  let m;
  while((m=re.exec(s))!==null)tokens.push(m[2]);
  if(tokens.length>0)return uniqueSorted(tokens);
  return original;
}

function normAns(v){
  if(v===null||v===undefined||v==="")return"";
  const choice=normChoice(v);
  if(/^[1-5](,[1-5])*$/.test(choice))return choice;
...
export default function App(){
      ...
      (async()=>{
        try{
          ...
          if(currentExam.folderId)params.set("folderId",currentExam.folderId);
            ...
            if(currentExam.setType||currentExam.round)params.set("setType",currentExam.setType||currentExam.round);
            if(currentExam.className)params.set("className",currentExam.className);
          }
  ...
  const hLookupExams=async()=>{
    ...
    try{
      ...
      for(const ex of exams){
        ...
        const tm=String(ex.examTime||ex.regTime||"");
        const tch=norm(ex.teacher);
        const key=cn
          ?`${cn}|${tch}|${ex.examType||""}|${ex.setType||ex.round||""}|${ex.examDate||""}|${tm}`
          :`_NOCN_|${norm(ex.level)}|${tch}|${ex.examType||""}|${ex.setType||ex.round||""}|${ex.examDate||""}|${tm}`;
        const prev=d1[key];
  ...
  const hPickExam=(ex)=>{
    ...
    const hasRealAns=!!(fixedAnswers&&Object.keys(fixedAnswers).some(k=>{
      const v=fixedAnswers[k];
      return v!==null&&v!==undefined&&String(v).trim()!=="";
    }));
    if(!hasRealAns){
      setCurrentExam(null);setAKey(null);setTKey(null);setALoad(false);setANF(true);setScr("info");
      alert("정답이 아직 준비되지 않았어요. 선생님이 AI 검수 확정 또는 직접 정답 저장을 완료한 뒤 다시 조회해주세요.");
      return;
    }
    ...
    if(hasRealAns)setALoad(true);else setANF(true);
  };
  const hShowHistory=()=>{
    if(!nm.trim())return alert("이름을 입력하세요.");
    if(!/^\d{4}$/.test(ph))return alert("핸드폰 뒷 4자리를 입력하세요.");
  ...
  const loadFeedback = async () => {
    ...
    try {
      ...
      if (h.folderId) params.set("folderId", h.folderId);
        ...
        if (h.className) params.set("className", h.className);
        if (h.setType || h.round) {
          params.set("setType", h.setType || h.round);
        } else {
          const setMatch = String(h.examName || "").match(/\(\s*([1-9]차|이론편|실전편|혼합|세트[A-E]|[A-E])\s*\)\s*$/);
          if (setMatch) params.set("setType", setMatch[1]);
        }
      }
